...
function simple_ajax_call(cmd,form_id,output_id,section,plugin)
{
    ...
    submit_data="section="+section+"&plugin="+plugin+"&type=simple"+"&cmd="+cmd;
    if (form_id!='')
    {
...
function complex_ajax_call(cmd, form_id,output_id,section,plugin)
{
    // This script will serialize the form indicated by an id and submit it
    // to the desired page.
    // Once the response has arrived it display the response inside the id
    ...
    submit_data="section="+section+"&plugin="+plugin+"&cmd="+cmd+"&type=complex&"+$('#'+form_id).serialize();
    //alert (submit_data);
    $.ajax({
               type: "POST",
               url: "index.php",
